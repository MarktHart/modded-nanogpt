...
class Hyperparameters:
    ...
    # optimization
    num_iterations = 25 # number of iterations to run
...
gradient_accumulation_steps = (original_seq_len * world_size_factor) // args.seq_len
...
assert gradient_accumulation_steps * args.seq_len * world_size == desired_world_size * original_seq_len, f"{gradient_accumulation_steps * args.seq_len=} != {desired_world_size * original_seq_len=}"
...
logfile = None
if master_process:
    run_id = f"{datetime.datetime.today().strftime('%y%m%d%H%M')}_{uuid.uuid4()}"
